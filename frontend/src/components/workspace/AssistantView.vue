...
const hasExternalToolbar = computed(() => !isPanel.value && !!props.headerActionHost);
...
<template>
  <div class="h-full flex flex-col min-h-0" :class="isPanel ? '' : (hasExternalToolbar ? 'gap-0' : 'gap-6')">
    ...
    <div
      :class="[
        ...
        isPanel ? '' : 'workspace-card',
        !isPanel && hasExternalToolbar ? 'mt-4' : '',
      ]"
    >
      <!-- Panel Variant：沿用卡片顶栏（侧栏内不走工作区操作栏） -->
      <div
        v-if="isPanel"
        class="bg-slate-50/80 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between backdrop-blur-sm px-3 py-3"
      >
        <div class="flex items-center gap-3 min-w-0">
          <div class="rounded-xl flex items-center justify-center text-white flex-shrink-0 w-9 h-9 bg-indigo-600 shadow-sm">
