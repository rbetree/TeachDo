...
const primaryLabel = computed(() => (props.hasPlan ? t('lesson.update') : t('lesson.generate')));
</script>
...
<template>
  <div class="flex-1 flex flex-col min-h-0">
    <div v-if="!props.externalToolbar" class="mb-6">
      <div class="min-w-0">
        <h2 class="text-2xl font-bold text-slate-900 dark:text-white">{{ t('lesson.choose_template') }}</h2>
        <p class="text-slate-500 dark:text-slate-400">{{ t('lesson.select_hint') }}</p>
      </div>
    </div>
    ...
    <div v-if="!props.externalToolbar" class="flex justify-center">
      <button
        ...
        @click="emit('primary')"
      >
        <LucideIcon :name="props.loading ? 'loader-2' : (props.hasPlan ? 'refresh-cw' : 'sparkles')" :size="20" :class="props.loading ? 'animate-spin' : ''" />
        <span>{{ props.loading ? t('common.loading') : primaryLabel }}</span>
      </button>
    </div>
  </div>
</template>
