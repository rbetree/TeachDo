...
### 5.1 前端路由（全标签路由化）
- `/`：课程列表。
- `/course/:courseId/unit/:unitId/:tab`。
- `tab` 取值：`outline`、`lesson`、`ppt`、`kb`、`assistant`。
...
### 阶段 B：工作台路由化
1. 将当前 tab 内部状态切换改为 URL 驱动切换。
2. 增加 `courseId/unitId/tab` 参数守卫与纠错回退。
3. 确保刷新后可以恢复到当前课程、单元和标签页。
- DoD：
