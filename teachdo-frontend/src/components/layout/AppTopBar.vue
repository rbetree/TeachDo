...
const statusText = computed(() => {
  if (isChecking.value) return t('nav.status.checking');
  if (backendConnected.value === null) return t('nav.status.system');
  return backendConnected.value ? t('nav.status.online') : t('nav.status.offline');
});
  ...
  <header class="h-16 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 fixed top-0 left-0 right-0 z-50 flex items-center">
    <div class="w-full px-4 md:px-6 flex items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-2 md:gap-3">
        <button
          class="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold border transition-colors transition-transform active:scale-95"
          :class="[
            isChecking
        ...
        <button
          class="p-2 rounded-full text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors font-bold text-xs flex items-center justify-center w-9 h-9 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-slate-900"
          type="button"
          :aria-label="t('a11y.toggle_language')"
          :title="t('a11y.toggle_language')"
        ...
        <button
          class="p-2 rounded-full text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-slate-900"
          type="button"
