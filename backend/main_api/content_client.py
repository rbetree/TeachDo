...
class A2AContentClientWrapper:
    def __init__(self, session_id: str, agent_url: str):
        ...
        self.task_id = None
        # 控制每个分片之间的轻微节流，默认 0.05 秒（可通过环境变量覆盖）
        self.part_delay_seconds = self._load_part_delay_seconds()

    def _load_part_delay_seconds(self) -> float:
        raw = os.environ.get("CONTENT_PART_DELAY_SECONDS", "0.05")
        try:
            return max(0.0, float(raw))
        except ValueError:
            self.logger.warning(
                "CONTENT_PART_DELAY_SECONDS=%s 不是合法数字，使用默认值 0.05",
                raw,
            )
            return 0.05
    ...
    async def setup(self) -> None:
        # 重要：内部服务（127.0.0.1/localhost）通信不应受系统代理环境变量影响，
        # 否则在设置了 http_proxy/https_proxy 且 no_proxy 未正确包含 127.0.0.1 时，
        # 会出现请求被转发到代理导致的 502/连接失败。
    ...
    async def generate(self, user_question: str, metadata={}) -> None:
        ...
        # 内容生成链路可能较长：读取超时设为 None，避免长思考阶段被客户端误判超时。
            ...
            try:
                ...
                async for chunk in stream_response:
                    ...
                    if "error" in chunk_data:
                        ...
                        yield {"type": "error", "text": str(chunk_data["error"]), "author": "system"}
                        break
                            ...
                            for part in parts:
                                ...
                                if part_kind == "data":
                                    ...
                                    for item in self.process_chart_part_text(part.get("text", ""), author):
                                        yield item
                                        # 轻微节流，减少前端粘包概率，同时避免大延时导致链路不稳定
                                        if self.part_delay_seconds > 0:
                ...
                self.logger.info("A2A 内容流被取消，session_id=%s", self.session_id)
                raise
            except Exception as e:
                # 上游 A2A 连接中断时，不直接抛出，返回结构化错误给 main_api 做 SSE 收尾。
                self.logger.error("A2A 内容流异常中断: %s", e, exc_info=True)
                yield {"type": "error", "text": f"A2A内容流异常中断: {e}", "author": "system"}
