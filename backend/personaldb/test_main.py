...
def _is_service_reachable(base_url: str) -> bool:
    """
    集成测试前探测服务是否可达。
    - 使用 trust_env=False，避免宿主机代理变量导致本地地址被走代理而误报 502。
    - 只要能拿到任意 HTTP 响应（哪怕 404）就认为“可达”。
    """
    probe_url = f"{base_url.rstrip('/')}/docs"
    try:
...
class KnowledgeBaseTestCase(unittest.TestCase):
    ...
    测试 FastAPI 接口
    """
    host = os.environ.get("HOST") or os.environ.get("host") or "127.0.0.1"
    if not host.startswith(("http://", "https://")):
        host = f"http://{host}"
    port = int(os.environ.get("PERSONAL_DB_PORT") or os.environ.get("port") or 9100)
    base_url = os.environ.get("PERSONAL_DB") or f"{host}:{port}"
    ...
    def setUpClass(cls) -> None:
        ...
        - 需要时设置：TEACHDO_INTEGRATION_TESTS=1，并确保 personaldb 已启动
        """
        ...
        if not _is_service_reachable(cls.base_url):
            raise unittest.SkipTest(f"跳过集成测试：personaldb 不可达 {cls.base_url}")
    ...
    def test_personal_db_search(self):
        ...
        try:
            # 发送POST请求
            with httpx.Client(timeout=httpx.Timeout(20.0), trust_env=False) as client:
...
对于特斯拉来说，Robotaxi平台的开放不仅意味着技术的进一步成熟和普及，更是对未来出行方式的一次重要探索。随着越来越多的用户加入测试行列，特斯拉将不断收集数据、优化算法，为未来的全面普及奠定坚实的基础。"""
        ...
        try:
            ...
            start_time = time.time()
            with httpx.Client(timeout=httpx.Timeout(40.0), trust_env=False) as client:
    ...
    def test_list_user_files(self):
        ...
        try:
            with httpx.Client(timeout=httpx.Timeout(20.0), trust_env=False) as client:
                response = client.get(list_url_with_files)
            response.raise_for_status()
