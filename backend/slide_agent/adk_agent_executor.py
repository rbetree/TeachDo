...
class ADKAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            logger.info("[adk executor] 请求已取消: task_id=%s", context.task_id)
            raise
        except Exception as e:
            # 异常时主动推送错误并完成任务，避免连接被直接中断导致上游 incomplete chunked read。
            logger.error("[adk executor] 执行异常: %s", e, exc_info=True)
            await updater.update_status(
                TaskState.working,
                message=updater.new_agent_message(
                    [TextPart(text=f"内容生成服务内部异常: {e}")],
                    metadata={"author": "system", "error": True},
                ),
            )
