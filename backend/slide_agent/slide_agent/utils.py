...
def parse_markdown_to_slides(markdown_text):
    ...
    for line in lines:
        if line.startswith('# '):
            ...
            slides.append({"type": "cover", "data": {"title": main_title, "text": cover_text_seed}})
            break
        ...
        if line.startswith('## '):
            if current_subsection_title:
                ...
                for item in current_items:
                    slide_items.append({"title": item, "text": content_text_seed})
                slides.append({"type": "content", "data": {"title": current_subsection_title, "items": slide_items}})
                current_items = []

            current_section_title = line[2:].strip()
            current_subsection_title = ""
            slides.append({"type": "transition", "data": {"title": current_section_title, "text": transition_text_seed}})
